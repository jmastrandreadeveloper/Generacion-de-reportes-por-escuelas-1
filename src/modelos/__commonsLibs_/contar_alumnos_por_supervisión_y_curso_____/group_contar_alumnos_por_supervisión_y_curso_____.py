...
def contar_alumnos_por_supervisión_y_curso______group(dataframe):
    required_columns = ['Supervisión', 'Curso ', 'Alumno_ID']
    ...
    result = dataframe.groupby(['Supervisión', 'Curso ']).agg({'Alumno_ID': 'count'})
    result.reset_index(inplace=True)
    result.rename(columns={'Alumno_ID': 'matricula_por_supervisión_y_curso'}, inplace=True)
    return result

def contar_alumnos_por_supervisión_y_curso______filter(CURSO_NORMALIZADO, dataframe, show_index=True, columns=None, orientacion='records', condiciones=None):
    try:
...
if __name__ == '__main__':
    CURSO_NORMALIZADO = '3°'  
    PATH_file = 'data/processed/transformed/Nominal/df_nominal_cantidad_de_alumnos_por_supervisión_y_curso.csv'
    ...
    condiciones = {
        "Supervisión": ["01 - Primario"],
    }

    data = contar_alumnos_por_supervisión_y_curso______filter(
        CURSO_NORMALIZADO, # la clave a buscar
        dataFrame, # el dataframe
        True, # se resetea el índice ? 
        ['Curso ', 'matricula_por_supervisión_y_curso' , ], # qué columnas quiero mostrar 
