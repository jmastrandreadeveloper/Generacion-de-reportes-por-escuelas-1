...
def filtrar_dataframe_nominales_de_alumnos_por_Supervisión(df, Supervisión, show_index=True, columns=None, orientacion='records', condiciones=None):
    """
    ...
    - Supervisión (int o str): El ID de la escuela que se quiere filtrar.
    ...
    Antes de filtrar, se revisa si Escuela_ID  están en el DataFrame. Si no existen, retorna una lista vacía [].
    ...
    """
    try:
        # Paso 1: Verificar si existen registros con el 'Supervisión' 
        if (Supervisión not in df['Supervisión'].unique()):
            return []  # Si no existen coincidencias, devuelve una lista vacía
        
        # Paso 2: Filtrar el DataFrame según los valores de 'Supervisión' 
        dFrame_filtrado = df[(df['Supervisión'] == Supervisión)]
